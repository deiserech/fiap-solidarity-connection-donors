...
namespace SolidarityConnection.Donors.Identity.Application.Services
{
    public class AuthService : IAuthService
    {
        ...
        public async Task<AuthResponseDto?> Register(RegisterDonorRequest registerDto)
        {
            ...
            {
                _logger.LogWarning("Registro falhou: email jÃ¡ existe: {Email}", registerDto.Email);
                return null;
            }

            if (await _userRepository.CpfExistsAsync(registerDto.Cpf))
            {
                _logger.LogWarning("Registro falhou: CPF jÃ¡ existe: {Cpf}", registerDto.Cpf);
                return null;
            }
            ...
            _logger.LogInformation("UsuÃ¡rio registrado com sucesso: {Email}", registerDto.Email);
            return new AuthResponseDto
            {
                Token = token,
                Email = user.Email,
        ...
        public async Task<AuthResponseDto?> CreateManager(CreateManagerRequest request)
        {
            ...
            {
                _logger.LogWarning("CriaÃ§Ã£o de manager falhou: email jÃ¡ existe: {Email}", request.Email);
                return null;
            }
